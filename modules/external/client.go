...
func GetSiteMetaData() ([]ExternalData ,error) {
	_, err := os.Stat(websites+"/info.json"); if err != nil {
	...
	file, err := os.OpenFile(websites+"/info.json", os.O_CREATE, 0755); if err != nil {
		fmt.Println(err)
	}
	defer file.Close()
	
	fsys := os.DirFS(websites)
	file1, err := fs.ReadFile(fsys, "info.json"); if err != nil {
			fmt.Println(err)
		}
	
err = json.Unmarshal(file1, &SiteData) ; if err != nil {
	fmt.Println(err)
}
...
func UpdateSiteMetaData() error {
	...
	client := http.DefaultClient
		res, err :=  client.Get("http://192.168.1.47:3000/api/v1/repos/search?uid=1"); if err != nil {
